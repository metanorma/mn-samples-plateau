...
[cols="4,9,9,9,9"]
.表 4-111　水部モデル（LOD0）の取得イメージ
|===
| 4+^| LOD0（河川ネットワークにより表現する場合）
| 取得例
2+a| image::images/385.webp["",150]
2+a| image::images/386.webp["",300]
...
| 4+^| LOD0（公共測量標準図式により表現する場合）
| 取得例
a| image::images/387.webp["",240]
a| image::images/388.webp["",100]
a| image::images/389.webp["",120]
a| image::images/390.webp["",200]
...
| 4+^| LOD0（公共測量標準図式により表現する場合）
| 取得例
a| image::images/391.webp["",120]
a| image::images/392.webp["",200]
a| image::images/393.webp["",100]
a| image::images/394.webp["",50]
...
[cols=2]
.表 4-112　水部モデル（LOD1）の取得イメージ
|===
| ^| LOD1
| 取得例
a| image::images/395.webp["",450]
...
[cols="3,47"]
.表 4-113　水部モデル（LOD2）の取得イメージ
|===
| ^| LOD2
| 取得例
a| image::images/396.webp["",600]
...
[cols=2]
.表 4-114　水部モデル（LOD3）の取得イメージ
|===
| ^| LOD3
| 取得例
a| image::images/397.webp["",500]
...
image::images/400.webp["",300,title=" 図　wtr:WaterBodyの例（LOD0）"]

2+a| 
LOD1は、水涯線により囲まれた面として表現する。

image::images/401.webp["",200,title=" 図　wtr:WaterBodyの例（LOD1）"]

2+a| 
LOD2は、水面と水底面に囲まれた立体として表現する。

image::images/402.webp["",400,title=" 図　wtr:WaterBodyの例（LOD2）"]

2+a| 
LOD3は、水面と水底面に囲まれた立体から、水中の構造物を除いた立体として表現する。

image::images/403.webp["",400,title=" 図　wtr:WaterBodyの例（LOD3）"]
...
| (wtr:function) | gml:CodeType [0..*] | 水部の種類。
...
image::images/404.webp["",500,title=" 図　水部の断面イメージ"]

| 上位の型 2+| wtr:_BoundarySurface
| ステレオタイプ 2+| << FeatureType >>
3+| 継承する属性
| 属性名 | 属性の型及び多重度 | 定義
...
3+| 自身に定義された属性
| 属性名 | 属性の型及び多重度 | 定義
| (wtr:waterLevel) | gml:CodeType [0..1] | 水位の状況。
3+| 継承する関連役割
| 関連役割名 | 関連役割の型及び多重度 | 定義
| (gen:stringAttribute) | gen:stringAttribute [0..*] | 文字列型属性。属性を追加したい場合に使用する。
| (gen:intAttribute) | gen:intAttribute [0..*] | 整数型属性。属性を追加したい場合に使用する。
...
image::images/405.webp["",500,title=" 図　水部の断面イメージ"]
...
image::images/406.webp["",500,title=" 図　水部の断面イメージ"]
...
===== 1)　uro:WaterBodyDetailAttribute
...
===== 1)　WaterBodyDetailAttribute_adminType.xml

[cols="3,22"]
|===
| ファイル名 | WaterBodyDetailAttribute_adminType.xml

| ファイルURL | https://www.geospatial.jp/iur/codelists/3.1/WaterBodyDetailAttribute_adminType.xml
| コード | 説明
| 0 | 不明
| 1 | 1級直轄区間
| 2 | 1級指定区間
