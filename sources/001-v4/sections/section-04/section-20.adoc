...
[cols="5,9"]
.表 4-116　区域モデル（LOD1）の取得イメージ
|===
| ^| LOD1
| 取得例
a| image::images/407.webp["",300]
...
| (gml:boundedBy) | gml:Envelope [0..1] | 区域の範囲と空間参照系。
...
| ファイルURL | https://www.geospatial.jp/iur/codelists/3.1/Zone_function.xml
| コード | 説明
| 0101 | 河川区域
| 0102 | 高規格堤防特別区域
| 0103 | 樹林帯区域
...
===== 2)　Common_validType.xml

都市計画決定情報のコードリストを参照。

　

===== 3)　Boundary_function.xml

都市計画決定情報のコードリストを参照。

　

===== 4)　Common_parkCode.xml

公園施設長寿命化計画のための拡張属性のコードリストを参照。

　

===== 5)　DataQualityAttribute_geometrySrcDesc.xml
...
===== 6)　DataQualityAttribute_thematicSrcDesc.xml
...
===== 7)　DataQualityAttribute_appearanceSrcDesc.xml
