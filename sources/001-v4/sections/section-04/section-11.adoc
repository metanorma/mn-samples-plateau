...
[cols="5,9,9,9,9,9"]
.表 4-60　橋梁モデル（LOD0）の取得イメージ
|===
| 5+^| LOD0
| 取得例
^a| image::images/231.png["",150]
^a| image::images/232.png["",120]
^a| image::images/233.png["",150]
^a| image::images/234.png["",120]
^a| image::images/235.png["",120]
...
[cols="4,9,9,9,9"]
.表 4-61　橋梁モデル（LOD1）の取得イメージ
|===
| 4+^| LOD1
| 取得例
^a| image::images/236.png["",190]
^a| image::images/237.png["",150]
^a| image::images/238.png["",150]
^a| image::images/239.png["",150]
...
image::images/240.png[title=" 図 4-5　橋梁の部材の名称"]

橋梁モデル（LOD2）の取得イメージを表 4-63に示す。

[cols="4,18,9,9"]
.表 4-63　橋梁モデル（LOD2）の取得イメージ
|===
| 3+^| LOD2.0
| 取得例
a| image::images/241.png["",250]
2+a| image::images/242.png["",350]
...
| 3+^| LOD2.1
| 取得例
a| image::images/243.png["",300]
a| image::images/244.png["",300]
a| image::images/245.png["",330]
...
[cols="3,3,6,8"]
.表 4-64　橋梁モデル（LOD3）に含むべき地物と対応するCityGMLの地物型
|===
2+a| その他の構造物モデル（LOD3） +
に含むべき地物
| 対応するCityGMLの地物型
...
[cols="2,9,9"]
.表 4-65　橋梁モデル（LOD3）の取得イメージ
|===
| 2+^| LOD3
| 取得例
a| image::images/246.png["",280]
a| image::images/247.png["",270]
...
| 2+^| LOD3
| 取得例
a| image::images/248.png["",260]
a| image::images/249.png["",370]
...
[cols="3,3,6,8"]
.表 4-66　橋梁モデル（LOD4）に含むべき地物
|===
2+| 橋梁モデル（LOD4）に含むべき地物 | 対応するCityGMLの地物型 ^| LOD4
2+| 橋梁 | Bridge ^| ●
2+| 橋梁部分
| BridgePart
^a| ■ +
...
[cols=3]
.表 4-67　（参考）CityGMLのクラスとIFCのクラスとの対応
|===
...
image::images/250.png[title=" 図 4-6　IfcBridgeにおけるクラス間の階層構造（出典：IFC Bridge Fast Track Project Report WP2: Conceptual Model）"]
...
| 上位の型 2+| brid:_AbstractBridge
| ステレオタイプ 2+| << FeatureType >>
3+| 継承する属性
| 属性名 | 属性の型及び多重度 | 定義
| (gml:description) | gml:StringOrRefType [0..1] | 橋梁の概要。
| gml:name | gml:CodeType [0..1] | 橋梁を識別する名称。文字列とする。
| (gml:boundedBy) | gml:Envelope [0..1] | 橋梁の範囲及び適用される空間参照系。
| core:creationDate | xs:date [0..1] | データが作成された日。運用上必須とする。
| core:terminationDate | xs:date [0..1] | データが削除された日。
| (core:relativeToTerrain) | core:RelativeToTerrainType [0..1] | 橋梁と地表面との相対的な位置関係。
| (core:relativeToWater) | core:RelativeToWaterType [0..1] | 橋梁と水面との相対的な位置関係。
| brid:class
| gml:CodeType [0..1]
...
| (brid:usage) | gml:CodeType [0..*] | 橋梁の用途。brid:functionで指定された機能と異なる場合に実際の用途を示すためにこの属性を用いる。
...
| (brid:lod1MultiSurface) | gml:MultiSurface [0..1] | 橋梁の外形を構成する面の集まり。
| (brid:lod1TerrainIntersection) | gml:MutiCurve [0..1] | LOD1における橋梁と地形との交線。
| brid:lod2Solid
| gml:Solid [0..1]
...
| (brid:lod2MultiCurve) | gml:MutiCurve [0..1] | 橋梁の立体表現に加え、線状の表現を行う場合に使用する。
| (brid:lod2TerrainIntersection) | gml:MutiCurve [0..1] | LOD2における橋梁と地形との交線。
...
| (brid:lod3MultiCurve) | gml:MutiCurve [0..1] | 橋梁の立体表現に加え、線状の表現を行う場合に使用する。これらの表現に意味を持たせる場合は、brid:BridgeInstallationやbrid:BridgeConstructionElementを使用する。
| (brid:lod3TerrainIntersection) | gml:MutiCurve [0..1] | LOD3における橋梁と地形との交線。
| brid:lod4Solid
| gml:Solid [0..1]
a| 橋梁の詳細構造の外形を示す立体。 +
この時の立体は、外壁等を区分する境界面及び開口部の面（境界面の内空として作成されている場合）により構成される。
...
| (brid:lod4MultiCurve) | gml:MutiCurve [0..1] | 橋梁の立体表現に加え、線状の表現を行う場合に使用する。
| (brid:lod4TerrainIntersection) | gml:MutiCurve [0..1] | LOD4における橋梁と地形との交線。
...
| (brid:address) | core:Address [0..*] | 橋梁に紐づけられた住所。
...
3+| 継承する属性
| 属性名 | 属性の型及び多重度 | 定義
| (gml:description) | gml:StringOrRefType [0..1] | 橋梁の部分の概要。
| gml:name | gml:CodeType [0..1] | 橋梁の部分を識別する名称。文字列とする。
| (gml:boundedBy) | gml:Envelope [0..1] | 橋梁の部分の範囲及び適用される空間参照系。
| core:creationDate | xs:date [0..1] | データが作成された日。運用上必須とする。
| core:terminationDate | xs:date [0..1] | データが削除された日。
| (core:relativeToTerrain) | core:RelativeToTerrainType [0..1] | 橋梁の部分と地表面との相対的な位置関係。
| (core:relativeToWater) | core:RelativeToWaterType [0..1] | 橋梁の部分と水面との相対的な位置関係。
| brid:class
| gml:CodeType [0..1]
a| 橋梁の形態による区分。コードリスト（Bridge_class.xml）より選択する。 +
多重度は任意となっているが、運用上必須とする。
...
| (brid:usage) | gml:CodeType [0..*] | 橋梁の部分の用途。brid:functionで指定された機能と異なる場合に実際の用途を示すためにこの属性を用いる。
...
| (brid:lod1MultiSurface) | gml:MultiSurface [0..1] | 橋梁部分の外形を構成する面の集まり。
| (brid:lod1TerrainIntersection) | gml:MutiCurve [0..1] | LOD1における橋梁の部分と地形との交線。
...
| (brid:lod2MultiCurve) | gml:MutiCurve [0..1] | 橋梁の部分の立体表現に加え、線状の表現を行う場合に使用する。
| (brid:lod2TerrainIntersection) | gml:MutiCurve [0..1] | LOD2における橋梁の部分と地形との交線。
...
| (brid:lod3MultiCurve) | gml:MutiCurve [0..1] | 橋梁の部分の立体表現に加え、線状の表現を行う場合に使用する。
| (brid:lod3TerrainIntersection) | gml:MutiCurve [0..1] | LOD3における橋梁の部分と地形との交線。
| brid:lod4Solid
| gml:Solid [0..1]
...
| (brid:lod4MultiCurve) | gml:MutiCurve [0..1] | 橋梁の部分の立体表現に加え、線状の表現を行う場合に使用する。
| (brid:lod4TerrainIntersection) | gml:MutiCurve [0..1] | LOD4における橋梁の部分と地形との交線。
| brid:interiorBridgeRoom | brid:InteriorBridgeRoom [0..*] | 橋梁の部分の内部空間（brid:InteriorBridgeRoom）
| (brid:consistsOfBridgePart) | brid:BridgePart [0..*] | 橋梁の部分の部分（brid:BridgePart）。
| (brid:address) | core:Address [0..*] | 橋梁の部分に紐づけられた住所。
...
| (uro:bridDisasterRiskAttribute) | uro:DisasterRiskAttribute [0..*] | 橋梁の部分の災害リスクに関する情報。
| (uro:bridKeyValuePairAttribute) | uro:KeyValuePairAttribute [0..*] | brid:BridgePartには作成しない。（brid:Bridgeにのみ作成する。）
| uro:bridDataQualityAttribute
| uro:DataQualityAttribute [0..1]
a| 作成するデータの品質に関する情報。 +
...
===== 3)　brid:BridgeConstructionElement
...
image::images/253.png["",600,title=" 図　brid:BridgeConstructionElementの例"]

| 上位の型 2+| brid:_CityObject
| ステレオタイプ 2+| << FeatureType >>
3+| 継承する属性
| 属性名 | 属性の型及び多重度 | 定義
| (gml:description) | gml:StringOrRefType [0..1] | 部材の概要。
| (gml:name) | gml:CodeType [0..1] | 部材を識別する名称。
| (gml:boundedBy) | gml:Envelope [0..1] | 部材の範囲及び適用される空間参照系。
...
3+| 自身に定義された関連役割
| 関連役割名 | 関連役割の型及び多重度 | 定義
| (brid:lod1Geometry) | gml:_Geometry [0..1] | 部材のLOD1の形状。
| brid:lod2Geometry
| gml:_Geometry [0..1]
a| 部材のLOD2の形状。 +
...
| (brid:usage) | gml:CodeType [0..*] | 区画の主な使い道。
3+| 継承する関連役割
| 関連役割名 | 関連役割の型及び多重度 | 定義
| (gen:stringAttribute) | gen:stringAttribute [0..*] | 文字列型属性。属性を追加したい場合に使用する。
| (gen:intAttribute) | gen:intAttribute [0..*] | 整数型属性。属性を追加したい場合に使用する。
...
| (brid:opening) | brid:_Opening [0..*] | 底面に設置される、窓や扉への参照。

|===

　

===== 8)　brid:OuterCeilingSurface
...
| (gen:measureAttribute) | gen:measureAttribute [0..*] | 単位付き数値型属性。属性を追加したい場合に使用する。
| (gen:genericAttributeSet) | gen:GenericAttributeSet [0..*] | 汎用属性のセット。属性を追加したい場合に使用する。
| (brid:lod2MultiSurface) | gml:MultiSurface [0..1] | 閉鎖面の外周に囲まれた面。
| brid:lod3MultiSurface | gml:MultiSurface [0..1] | 閉鎖面の外周に囲まれた面。
| brid:lod4MultiSurface | gml:MultiSurface [0..1] | 閉鎖面の外周に囲まれた面。
| (brid:opening) | brid:_Opening [0..*] | 境界面に設置される、窓や扉への参照。

|===

===== 11)　brid:InteriorWallSurface
...
| (brid:usage) | gml:CodeType [0..*] | 内部付属物の主な使い道。
3+| 継承する関連役割
| 関連役割名 | 関連役割の型及び多重度 | 定義
| (gen:stringAttribute) | gen:stringAttribute [0..*] | 文字列型属性。属性を追加したい場合に使用する。
| (gen:intAttribute) | gen:intAttribute [0..*] | 整数型属性。属性を追加したい場合に使用する。
...
===== 1)　uro:BridgeStructureAttribute
...
===== 2) uro:BridgeFunctionalAttribute
...
| ファイルURL | https://www.geospatial.jp/iur/codelists/3.1/Bridge_class.xml
| コード | 説明
| 01 | 桁橋
| 02 | トラス橋
| 03 | アーチ橋
...
===== 2) Bridge_function.xml

[cols="3,22"]
|===
| ファイル名 | Bridge_function.xml

| ファイルURL | https://www.geospatial.jp/iur/codelists/3.1/Bridge_function.xml
| コード | 説明
| 01 | 道路橋
| 02 | 鉄道橋
| 03 | 水路橋
...
===== 3) BridgeConstructionElement_function.xml

[cols="3,22"]
|===
| ファイル名 | BridgeConstructionElement_function.xml

| ファイルURL | https://www.geospatial.jp/iur/codelists/3.1/BridgeConstructionElement_function.xml
| コード | 説明
| 01 | 橋脚
| 02 | 橋台
| 03 | トラス
...
===== 4) BridgeInstallation_function.xml

[cols="3,22"]
|===
| ファイル名 | BridgeInstallation_function.xml

| ファイルURL | https://www.geospatial.jp/iur/codelists/3.1/BridgeInstallation_function.xml
| コード | 説明
| 01 | 支承
| 02 | 落橋防止装置
| 03 | 伸縮装置
...
===== 1) BridgeStructureAttribute_material.xml

[cols="3,22"]
|===
| ファイル名 | BridgeStructureAttribute_material.xml

| ファイルURL | https://www.geospatial.jp/iur/codelists/3.1/BridgeStructureAttribute_material.xml
| コード | 説明
| 01 | 鋼溶接橋
| 02 | 鋼（鉄）リベット橋
| 03 | RC橋
...
===== 2) BridgeStructureAttribute_bridgeType.xml

[cols="3,22"]
|===
| ファイル名 | BridgeStructureAttribute_bridgeType.xml

| ファイルURL | https://www.geospatial.jp/iur/codelists/3.1/BridgeStructureAttribute_bridgeType.xml
| コード | 説明
| 01 | 橋
| 02 | 高架橋
| 03 | 桟道橋
...
===== 3) BridgeFunctionalAttribute_userType.xml

[cols="3,22"]
|===
| ファイル名 | BridgeFunctionalAttribute_userType.xml

| ファイルURL | https://www.geospatial.jp/iur/codelists/3.1/BridgeFunctionalAttribute_userType.xml
| コード | 説明
| 01 | 歩行者専用
| 02 | 自転車専用
| 03 | 自転車歩行者専用
...
===== 4) ConstructionStructureAttribute_slopeType.xml
...
===== 5) ConstructionBaseAttribute_adminType.xml
...
===== 6) ConstructionBaseAttribute_installerType.xmlりんりりｎ
...
===== 7) ConstructionRiskAssessmentAttribute_riskType.xml
...
===== 8) ConstructionRiskAssessmentAttribute_status.xml
...
===== 9) DataQualityAttribute_geometrySrcDesc.xml
...
===== 10) DataQualityAttribute_thematicSrcDesc.xml
...
===== 11) DataQualityAttribute_appearanceSrcDesc.xml
...
===== 12) DataQualityAttribute_lod1HeightType.xml
...
===== 13) PublicSurveyDataQualityAttribute_srcScale.xml
...
===== 14) PublicSurveyDataQualityAttribute_geometrySrcDesc.xml
...
===== 15) Bridge_lodType.xml

[cols="3,22"]
|===
| ファイル名 | Bridge_lodType.xml

| ファイルURL | https://www.geospatial.jp/iur/codelists/3.1/Bridge_lodType.xml
| コード | 説明
