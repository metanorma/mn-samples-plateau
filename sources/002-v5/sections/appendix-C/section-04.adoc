...
同様にして、bldg:BuildingPartはデータ品質属性（uro:DataQualityAttribute）をもつことができる。この場合、これを部品とするbldg:Buildingはデータ品質属性をもたない。また、データ品質属性の各属性には、bldg:BuildingPartのデータ作成に使用した原典資料の地図情報レベル、その他原典資料の諸元を記述することが基本となる。ただし、uro:geometrySrcDescLod0、uro:lod1HeightType等のLOD0及びLOD1の幾何オブジェクトについての属性は、このbldg:BuildingPartを部品として持つbldg:BuildingのLOD0及びLOD1の幾何オブジェクトに対応する値を記述する。

なお、bldg:Building の部品となる全てのbldg:BuildingPartについて、データ品質属性の値が全て等しくなる場合は、bldg:Buildingにデータ品質属性を記述することができる。このとき、部品となる各bldg:BuildingPartはデータ品質属性をもたない。
